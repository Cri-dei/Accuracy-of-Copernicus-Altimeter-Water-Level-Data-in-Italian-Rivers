...
#Load in situ data
 ...
 HYDRO_data_ALL<- append(HYDRO_data_PO,HYDRO_data_TOSCANA_managed_ok)
...
#Aggiungere linea per togliere a sent hydro data all le posizioni
 ...
 { 
      if(ii<5){
        ...
        if(ii>=5 && ii<=8)
         ...
         { HYDRO_data_ALL2[[ii]]$DATAORA<-as.POSIXct(paste(HYDRO_data_ALL2[[ii]]$Data_format, HYDRO_data_ALL2[[ii]]$Ora), format="%Y%m%d %H:%M")}
 ...
 for (xx in 1:length(HYDRO_data_ALL2))
 {      pos<-which(names(HYDRO_data_ALL2)==names(SENT_HYDRO_data_ALL[xx]))
 
 
       if(is.list(HYDRO_data_ALL2[[pos]][[1]])=="FALSE")
       {
        Y_start<-which(HYDRO_data_ALL2[[pos]]$Data_format==as.numeric(as.character(SENT_HYDRO_data_ALL[[1]]$Data_format[1])))[1] 
        Y_start<- Y_start-20
