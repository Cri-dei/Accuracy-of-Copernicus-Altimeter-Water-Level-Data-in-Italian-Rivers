...
{
  SENT_HYDRO_data_ALL[[jj]]$Diff_HydSent<-SENT_HYDRO_data_ALL[[jj]]$Hydro_diff - SENT_HYDRO_data_ALL[[jj]]$Sentinel_diff
  
}


DIFFERENCES<-lapply(1:length(SENT_HYDRO_data_ALL),function(x){
  SENT_HYDRO_data_ALL[[x]]$Hydro_diff - SENT_HYDRO_data_ALL[[x]]$Sentinel_diff })

names(DIFFERENCES)<-seq(1:length(DIFFERENCES))
...
save(SENT_HYDRO_data_ALL,DIFFERENCES, file="SENT_HYDRO_data_ALL_diff_v2.RData")
...
Summary_results<-data.frame(matrix(,length(SENT_HYDRO_data_ALL),14))
colnames(Summary_results)<-c("Code","Station","Latitude","Longitude",
                             "Correlation","Rsquared","RMSE","MBE","Section","Distance","Region","River","Identifier","Satellite")

for ( jj in 1:length(SENT_HYDRO_data_ALL))
{
  gh_pos<-which(Merged_STAZIONI_ALL_INFO$NomeStazione2==names(SENT_HYDRO_data_ALL[jj]))
  Summary_results$Code[[jj]]<-jj
  Summary_results$Station[[jj]]<- Merged_STAZIONI_ALL_INFO$`Stazione`[gh_pos]
  Summary_results$Latitude[[jj]]<- round( Merged_STAZIONI_ALL_INFO$`Lat`[gh_pos],2)
  Summary_results$Longitude[[jj]]<- round(Merged_STAZIONI_ALL_INFO$`Long`[gh_pos],2)
  ...
  na0<-which(is.na(SENT_HYDRO_data_ALL[[jj]]$Hydro_diff))
      if(length(na0)>0){
    Summary_results$RMSE[[jj]]<-   rmse(SENT_HYDRO_data_ALL[[jj]]$Hydro_diff[-na0], SENT_HYDRO_data_ALL[[jj]]$Sentinel_diff[-na0])
    Summary_results$MBE[[jj]]<-   tdStats(SENT_HYDRO_data_ALL[[jj]]$Hydro_diff[-na0], SENT_HYDRO_data_ALL[[jj]]$Sentinel_diff[-na0], functions = c("mbe"))
    } else{ 
    Summary_results$RMSE[[jj]]<-   rmse(SENT_HYDRO_data_ALL[[jj]]$Hydro_diff, SENT_HYDRO_data_ALL[[jj]]$Sentinel_diff)
    Summary_results$MBE[[jj]]<-   tdStats(SENT_HYDRO_data_ALL[[jj]]$Hydro_diff, SENT_HYDRO_data_ALL[[jj]]$Sentinel_diff, functions = c("mbe"))
    }
  
  Summary_results$Section[[jj]]<-  Merged_STAZIONI_ALL_INFO$`Larghezza.Alveo..m.`[gh_pos]
  Summary_results$Region[[jj]]<-  Merged_STAZIONI_ALL_INFO$Regione[gh_pos]
  Summary_results$Distance[[jj]]<-  Merged_STAZIONI_ALL_INFO$Distance[gh_pos]
  Summary_results$Identifier[[jj]]<-  Merged_STAZIONI_ALL_INFO$SENTINEL[gh_pos]
  Summary_results$Satellite[[jj]]<-  as.character(Merged_STAZIONI_ALL_INFO$Satellite[gh_pos])
  Summary_results$River[[jj]]<- as.character(Merged_STAZIONI_ALL_INFO$Corso.d.acqua[gh_pos])
 }
...
pdf("Sentinel_results_25112020_V2.pdf",onefile = TRUE)
