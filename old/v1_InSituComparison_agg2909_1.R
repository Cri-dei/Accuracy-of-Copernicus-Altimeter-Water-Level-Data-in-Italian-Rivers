...
for (xx in 1:length(VENETO_COUPLES)+1)
{
  Pos_VENETO<-which(names(HYDRO_data_VENETO_managed[xx])== STAZIONI_ALL_INFO$NomeStazione)
  
  
  if(length(Pos_VENETO)>1)
  { 
    for(gg in 1: length(Pos_VENETO))
    {
      Pos_VENETO_1<-Pos_VENETO[gg]
      PosTsent<-which(Sentinel3_data_Info$Identifier==STAZIONI_ALL_INFO$SENTINEL[Pos_VENETO_1])
  ...
  SENT_HYDRO_data_VENETO[[xx]]<-data.frame( merge(Sentinel3_data_managed[[PosTsent]],HYDRO_data_VENETO_managed[[xxx]],
...
load("C:/Users/39349/Documents/Remote Sensing/In-situ data/Veneto/HYDRO_data_VENETO.RData")
load("C:/Users/39349/Documents/Remote Sensing/In-situ data/Toscana/HYDRO_data_TOSCANA.RData")
...
pdf("ALL STATIONS_250920.pdf",onefile = TRUE)
