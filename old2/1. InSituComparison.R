...
for (xx in 1:length(Sentinel3_data))
{
  ...
  Hour_format0<-as.POSIXct(Sentinel3_data[[xx]][,5], format="%d-%m-%Y-%H:%M",tz="UTC")

  Hour_format_ora<-lubridate::round_date(Hour_format0, "1 hour")   
  
  Hour_format_15min0<-lubridate::round_date(Hour_format0, "15 minutes") 
  
  Hour_format_30min0<-lubridate::round_date(Hour_format0, "30 minutes")   
...
for (xx in 1:length(HYDRO_data_PO))
{ 
  Pos_statpo<-which(names(HYDRO_data_PO[xx])== STAZIONI_ALL_INFO$NomeStazione)
  Pos_sentpo<-which(Sentinel3_data_Info$Identifier==STAZIONI_ALL_INFO$SENTINEL[Pos_statpo])
  
  
  
  SENT_HYDRO_data_PO[[xx]]<-data.frame( merge(Sentinel3_data_managed[[Pos_sentpo]],HYDRO_data_PO[[xx]],
...
{ 
 ...
 rsq2 <- function (x, y) cor(x, y) ^ 2
 
################################## PLOT PER ALLL ########################################

pdf("ALL STATIONS_290920_prova5.pdf",onefile = TRUE)
