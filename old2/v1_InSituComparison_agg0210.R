...
for (xx in 1:length(Sentinel3_data))
{
  ...
  Hour_format_15min<-sapply(1:nrow(Sentinel3_data[[xx]]),function(x){ 
    h<-as.numeric(as.character(substring(Sentinel3_data[[xx]][x,5],12,13)))
    minutes<-substring(Sentinel3_data[[xx]][x,5],15,16)
    round_any(as.numeric(paste0(h,minutes)), 5) })
  
  Hour_format_30min<-sapply(1:nrow(Sentinel3_data[[xx]]),function(x){ 
    h<-as.numeric(as.character(substring(Sentinel3_data[[xx]][x,5],12,13)))
    minutes<-round_any(as.numeric(substring(Sentinel3_data[[xx]][x,5],15,16)), 30)
    ifelse(minutes==60,(h+1)*10, as.numeric(paste0(h,minutes)))
     })
  
  Sentinel3_data_managed[[xx]]<-cbind(Sentinel3_data[[xx]], Data_format, Hour_format,Hour_format_15min,Hour_format_30min)
...
{ 
  Pos_statL<-which(names(HYDRO_data_LAZIO_managed[xx])== STAZIONI_ALL_INFO$NomeStazione)
  Pos_sentL<-which(Sentinel3_data_Info$Identifier==STAZIONI_ALL_INFO$SENTINEL[Pos_statL])
  ...
  SENT_HYDRO_data_LAZIO[[xx]]<-merge(HYDRO_data_LAZIO_managed[[Pos_statL]],Sentinel3_data_managed[[Pos_sentL]], by.x = c("Data_format","Hour_format"), by.y = c( "Data_format","Hour_format_15min")) 
...
VENETO_COUPLES<-which(STAZIONI_ALL_INFO$Regione=="VENETO" & STAZIONI_ALL_INFO$SENTINEL>0)
...
names(SENT_HYDRO_data_VENETO)<-STAZIONI_ALL_INFO$NomeStazione[VENETO_COUPLES]
...
load("C:/Users/39349/Documents/Remote Sensing/In-situ data/Veneto/SENT_HYDRO_data_VENETO.RData")
load("C:/Users/39349/Documents/Remote Sensing/In-situ data/Toscana/SENT_HYDRO_data_TOSCANA.RData")
...
Summary_results<-data.frame(matrix(,length(SENT_HYDRO_data_ALL),4))
colnames(Summary_results)<-c("Station","Correlation","Section","Region")
...
{ 
  ...
  Summary_results$Region[[jj]]<-  STAZIONI_ALL_INFO$Regione[gh_pos[1]]
...
pdf("ALL STATIONS_290920_3.pdf",onefile = TRUE)
...
{ 
  ...
  p1_ALL<-p1_ALL +theme(axis.text.x = element_text(face="bold", color="#993333",  size=6, angle=45))+
    ggtitle(paste(Summary_results$Region[jj], "-Stazione:", names(SENT_HYDRO_data_ALL)[[jj]],Summary_results$Section[jj]) )+
